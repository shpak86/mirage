...
- `--proxy string`  
  URL HTTP-прокси (например, `socks5://127.0.0.1:8080`).
  Если указан, все запросы будут отправляться через прокси.
  По умолчанию: без прокси.

- `--version string`  
  Версия HTTP-протокола (`1.1`, `2`, `3`).
  Если не указана, версия определяется автоматически на основе фингерпринта и возможностей сервера.
  По умолчанию: auto.
